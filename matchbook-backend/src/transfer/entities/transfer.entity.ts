...
export class Transfer {
    ...
    @ManyToOne(() => Publication, { onDelete: 'SET NULL', onUpdate: 'CASCADE' })
    @JoinColumn({ name: 'id_publication' })
    publication: Publication;

    @ManyToOne(() => StatusSend, { onDelete: 'SET NULL', onUpdate: 'CASCADE' })
    @JoinColumn({ name: 'id_status_send' })
    statusSend: StatusSend;

    @ManyToOne(() => ShipmentType, { onDelete: 'SET NULL', onUpdate: 'CASCADE' })
